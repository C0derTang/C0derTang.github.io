...
export function interiorFrameLayer(): Layer {
  const post = (x: number) =>
    rect(x, -220, 64, 1640, 'url(#in-post)') +
    texRect('woodV', x, -220, 64, 1640, 64, 0.3, 256) +
    rect(x, -220, 6, 1640, v('wood-mid'), 'opacity=".5"')
  const rafters = [500, 800, 1100]
    ...
    .join('')
  const defs = `<defs>${linGrad('in-void', [
    [0, '#0d0906'],
    [1, '#1e1512'],
  ])}${radGrad('in-beamGlow', [
    [0, v('lamp-glow'), 0.2],
...
/**
 * The room on three depths: back wall pieces + shelf, the tatami floor with its shading and
 * the lamp glow, the hearth and cushion. The animated props live in interiorPropsLayer.
 */
export function interiorRoomLayer(): Layer {
  ...
  rows.forEach(([y0, y1], ri) => {
    for (let c = 0; c < 3; c++) {
      ...
      mats += polygon(pts, odd ? 'url(#in-matB)' : 'url(#in-matA)')
      let weave = ''
      for (let k = 1; k < 6; k++) {
  ...
  const tatami = `
    ...
    ${circle(LAMP.x, LAMP.y, 460, 'url(#in-lampGlow)')}
...
/** Kettle on its chain, the carp counterweight, steam, the lamp with its flicker: the room's animated props (live plane). */
